    ...
    player_onice <- tryCatch(player_data_clean %>%
      ...
      season_bystrength_game <- player_shifts_data %>%
          ...
          summarise(#Games_Played = uniqueN(Game_Id),
                ...
                #OTF_Shift_Share = sum(OTF_Shift, na.rm = TRUE) / Total_Shifts,
                #Off_Shift_Share = sum(Off_FO_Shift, na.rm = TRUE) / Total_Shifts,
                #Def_Shift_Share = sum(Def_FO_Shift, na.rm = TRUE) / Total_Shifts,
                #Neu_Shift_Share = sum(Neu_FO_Shift, na.rm = TRUE) / Total_Shifts,
                
                #ixG60 = sum(ixG, na.rm = TRUE) / (sum(TOI) / 3600),
                #G60 = sum(G, na.rm = TRUE) / (sum(TOI) / 3600),
                #P160 = (sum(G, na.rm = TRUE) + sum(A1, na.rm = TRUE)) / (sum(TOI) / 3600),
                #P60 = (sum(G, na.rm = TRUE) + sum(A1, na.rm = TRUE) + sum(A2, na.rm = TRUE))  / (sum(TOI) / 3600),
                ...
                #xGF60 = sum(xGF, na.rm = TRUE) / (sum(TOI)/ 3600),
                #xGA60 = sum(xGA, na.rm = TRUE) / (sum(TOI) / 3600),
                #SF60 = sum(SF, na.rm = TRUE) / (sum(TOI) / 3600),
                #SA60 = sum(SA, na.rm = TRUE) / (sum(TOI) / 3600),
...
sm_shift <- skater_season_stats(i, szns = c("20162017"))
